...
# Устанавливаем Bash версии - bash=5.1.0-r0
...
* Во-вторых, `RUN apk add --no-cache bash=5.1.0-r0` - здесь мы используем пакетный менеджер Alpine (`apk`) для установки только конкретной версии Bash. Так мы изебгаем обновления всех пактов в системе, что может привести к непредсказуемому поведению в работе контейнера, и добавляем тоелько то, что надо для реализации нашего приложения. Также при помощи флага `--no-cache` мы убираем сохранение кэша, что уменьшает размер контенера.
